...
if __name__ == "__main__":
    ...
    # history of gains over look back period (0 if no gain, magnitude of gain if gain)
    gain_history = []
    # history of losses over look back period (0 if no loss, magnitude of loss if loss)
    loss_history = []
    ...
    # current_price - last_price > 0 => gain. current_price - last_price < 0 => loss.
    last_price = 0
    ...
    goog_data = goog_data.assign(
        RelativeStrengthAvgGainOver20Days=pd.Series(avg_gain_values, index=goog_data.index)
    )
    goog_data = goog_data.assign(
        RelativeStrengthAvgLossOver20Days=pd.Series(avg_loss_values, index=goog_data.index)
    )
    goog_data = goog_data.assign(
        RelativeStrengthIndicatorOver20Days=pd.Series(rsi_values, index=goog_data.index)
    )
    ...
    close_price.plot(ax=ax1, color="black", lw=2.0, legend=True)
