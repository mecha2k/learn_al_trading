...
if __name__ == "__main__":
    ...
    goog_monthly_sample = goog_data["Adj Close"].pct_change().mul(100).resample("1M").mean()
    goog_monthly_sample = goog_monthly_sample.groupby(
        [goog_monthly_sample.index.year, goog_monthly_sample.index.month]
    ).mean()
    goog_monthly_sample.index.names = ["year", "month"]
    goog_monthly_sample = goog_monthly_sample.to_frame()
    print(goog_monthly_sample.head())

    print(goog_monthly_return.index[0][1])
    print(goog_monthly_return.iloc[0])
    ...
    # plt.title("GOOG Montly return 2001-2020")
    # # plt.suptitle("")
    # plt.show()
    # #
    # fig = plt.figure()
